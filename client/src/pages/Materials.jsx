...
const Materials = () => {
  ...
  const handleAddMaterial = async (e) => {
    e.preventDefault();
    if (!formData.name || !formData.supplierId || !formData.quantity || !formData.totalAmount) {
    ...
    try {
      ...
      setFormData({ ...formData, name: '', quantity: '', totalAmount: '', invoiceNumber: '' });
      fetchMaterials();
    } catch (error) {
  ...
  return (
    <div>
            ...
            <tbody>
              {materials.map(mat => (
                <tr key={mat._id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                  ...
                  <td style={{ padding: '1rem' }}>{mat.supplierName || 'Unknown'}</td>
      ...
      <Modal 
        ...
        title="Add Direct Material Entry"
        footer={
          <>
            <Button variant="secondary" onClick={() => setIsModalOpen(false)}>Cancel</Button>
            <Button onClick={handleAddMaterial} isLoading={submitting}>Save Entry</Button>
          </>
        }
      >
